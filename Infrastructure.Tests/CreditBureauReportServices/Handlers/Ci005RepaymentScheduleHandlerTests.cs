...
public class Ci005RepaymentScheduleHandlerTests
{
    private static (
        Ci005RepaymentScheduleHandler Sut,
        ...
        Mock<ITelegramNotificationService> Telegram) CreateSut()
    {
        ...
        var apiOptions = new CreditBureauApiOptions
        {
            ...
            CreditScheduleUrl = "/credit/registration/schedule"
        };
        ...
        var sut = new Ci005RepaymentScheduleHandler(
            repository.Object, requestManager.Object, reportOptions, apiOptions, security, bankHeader, logWriter,
            telegram.Object, new Mock<ILogger<Ci005RepaymentScheduleHandler>>().Object);
    ...
    [Fact]
    public async Task ProcessAsync_ReportsCiCodeFive()
    {
        var (sut, _, repository, _) = CreateSut();
        repository.Setup(r => r.CreditRegistrationRepaymentSchedulesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([]);

        Assert.Equal(5, sut.CiCode);
    ...
    public async Task ProcessAsync_WhenRequestIsNull_MarksErrorAndNotifiesWithoutCallingBureau()
    {
        ...
        repository.Verify(r => r.UpsertCiStatusAsync(1, 5, 2, "CI-005 request is null", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-005 null request", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenBureauReturnsSuccess_MarksSuccessWithoutNotifying()
    {
        var (sut, requestManager, repository, telegram) = CreateSut();
        repository.Setup(r => r.CreditRegistrationRepaymentSchedulesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([new() { LoanKey = 2, Request = new CreditRegistrationRepaymentSchedule { PPlanArray = [] } }]);
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/schedule", It.IsAny<string>(), 2, It.IsAny<CancellationToken>()))
            .ReturnsAsync(SuccessResponse);

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Success);
        repository.Verify(r => r.UpsertCiStatusAsync(2, 5, 1, "ok", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
    ...
    public async Task ProcessAsync_WhenBureauReturnsErrorResult_MarksErrorAndNotifies()
    {
        var (sut, requestManager, repository, telegram) = CreateSut();
        repository.Setup(r => r.CreditRegistrationRepaymentSchedulesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([new() { LoanKey = 3, Request = new CreditRegistrationRepaymentSchedule { PPlanArray = [] } }]);
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/schedule", It.IsAny<string>(), 3, It.IsAny<CancellationToken>()))
            .ReturnsAsync("""{"result":"05555","resultMessage":"Freeze active"}""");

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Error);
        repository.Verify(r => r.UpsertCiStatusAsync(3, 5, 2, "Freeze active", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-005 API error", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenResponseIsEmpty_MarksErrorAndNotifies()
    {
        var (sut, requestManager, repository, telegram) = CreateSut();
        repository.Setup(r => r.CreditRegistrationRepaymentSchedulesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([new() { LoanKey = 4, Request = new CreditRegistrationRepaymentSchedule { PPlanArray = [] } }]);
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/schedule", It.IsAny<string>(), 4, It.IsAny<CancellationToken>()))
            .ReturnsAsync(string.Empty);

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Error);
        repository.Verify(r => r.UpsertCiStatusAsync(4, 5, 2, "CI-005 returned empty response", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-005 empty response", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenSendPostRequestThrows_MarksErrorAndNotifiesWithoutStoppingQueue()
    {
        ...
        repository.Setup(r => r.CreditRegistrationRepaymentSchedulesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([
                new() { LoanKey = 5, Request = new CreditRegistrationRepaymentSchedule { PPlanArray = [] } },
                new() { LoanKey = 6, Request = new CreditRegistrationRepaymentSchedule { PPlanArray = [] } }
            ]);
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/schedule", It.IsAny<string>(), 5, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new InvalidOperationException("network down"));
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/schedule", It.IsAny<string>(), 6, It.IsAny<CancellationToken>()))
            .ReturnsAsync(SuccessResponse);
        ...
        Assert.Equal(1, result.Success);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-005 processing exception", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
}
