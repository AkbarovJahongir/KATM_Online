...
public class Ci015RepaymentRequestHandlerTests
{
    private static (
        Ci015RepaymentRequestHandler Sut,
        ...
        Mock<ITelegramNotificationService> Telegram) CreateSut()
    {
        ...
        var apiOptions = new CreditBureauApiOptions
        {
            ...
            CreditRegistrationRepaymentUrl = "/credit/registration/repayment"
        };
        ...
        var sut = new Ci015RepaymentRequestHandler(
            repository.Object, requestManager.Object, reportOptions, apiOptions, security, bankHeader, logWriter,
            telegram.Object, new Mock<ILogger<Ci015RepaymentRequestHandler>>().Object);

        return (sut, requestManager, repository, telegram);
    }

    private static CreditRegistrationRepayment NewRequest() => new() { PRepaymentArray = [] };

    private const string SuccessResponse = """{"result":"00000","resultMessage":"ok"}""";

    [Fact]
    public async Task ProcessAsync_ReportsCiCodeFifteen()
    {
        var (sut, _, repository, _) = CreateSut();
        repository.Setup(r => r.GetCreditRegistrationRepaymentRequestsAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync([]);

        Assert.Equal(15, sut.CiCode);
    ...
    public async Task ProcessAsync_WhenRequestIsNull_MarksErrorAndNotifiesWithoutCallingBureau()
    {
        ...
        repository.Verify(r => r.UpsertCiStatusAsync(1, 15, 2, "CI-015 request is null", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-015 null request", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenBureauReturnsSuccess_MarksSuccessWithoutNotifying()
    {
        ...
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 2, It.IsAny<CancellationToken>()))
            .ReturnsAsync(SuccessResponse);

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Success);
        repository.Verify(r => r.UpsertCiStatusAsync(2, 15, 1, "ok", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
    ...
    public async Task ProcessAsync_WhenBureauReturnsErrorResult_MarksErrorAndNotifies()
    {
        ...
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 3, It.IsAny<CancellationToken>()))
            .ReturnsAsync("""{"result":"05555","resultMessage":"Freeze active"}""");

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Error);
        repository.Verify(r => r.UpsertCiStatusAsync(3, 15, 2, "Freeze active", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-015 API error", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenResponseIsEmpty_MarksErrorAndNotifies()
    {
        ...
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 4, It.IsAny<CancellationToken>()))
            .ReturnsAsync(string.Empty);

        var result = await sut.ProcessAsync(CancellationToken.None);

        Assert.Equal(1, result.Error);
        repository.Verify(r => r.UpsertCiStatusAsync(4, 15, 2, "CI-015 returned empty response", null, It.IsAny<CancellationToken>()), Times.Once);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-015 empty response", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ProcessAsync_WhenSendPostRequestThrows_MarksErrorAndNotifiesWithoutStoppingQueue()
    {
        ...
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 5, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new InvalidOperationException("network down"));
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 6, It.IsAny<CancellationToken>()))
            .ReturnsAsync(SuccessResponse);
        ...
        Assert.Equal(1, result.Success);
        telegram.Verify(t => t.NotifyErrorAsync(
            "CI-015 processing exception", It.IsAny<string>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task SendByPeriodAsync_UsesPeriodQueueWithGivenDatesAndLoanKey()
    {
        ...
        repository.Setup(r => r.GetCreditRegistrationRepaymentRequestsByPeriodAsync(start, end, 7, It.IsAny<CancellationToken>()))
            .ReturnsAsync([new() { LoanKey = 7, Request = NewRequest() }]);
        requestManager.Setup(r => r.SendPostRequest(
                "https://bureau.test/credit/registration/repayment", It.IsAny<string>(), 7, It.IsAny<CancellationToken>()))
            .ReturnsAsync(SuccessResponse);

        var result = await sut.SendByPeriodAsync(start, end, 7, CancellationToken.None);

        Assert.Equal(1, result.Success);
        repository.Verify(r => r.GetCreditRegistrationRepaymentRequestsByPeriodAsync(start, end, 7, It.IsAny<CancellationToken>()), Times.Once);
        repository.Verify(r => r.UpsertCiStatusAsync(7, 15, 1, "ok", null, It.IsAny<CancellationToken>()), Times.Once);
    }
}
