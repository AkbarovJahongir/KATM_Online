...
{
    public class CreditBureauRepository(DatabaseSettings databaseSettings) : ICreditBureauRepository
    {
        ...
        public async Task<List<LoanApplication>> GetLoanApplications(CancellationToken cancellationToken)
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                resultData.Add(new LoanApplication()
                {
                    ...
                    PReportId = reader["pReportId"] is DBNull ? null : reader["pReportId"].ToString(),
        ...
        public async Task UpdateRequestHistoryXmlStatusAsync(string keyCreditBureauKb, string status, CancellationToken cancellationToken)
        {
            using var connect = new SqlConnection(_databaseSettings.CIBConnection);
            using var cmd = new SqlCommand(
                "UPDATE [dbo].[Request_History_Xml] SET [status] = @status WHERE [Key_ABS_Loan] = @keyCreditBureauKb",
                connect);
            cmd.Parameters.AddWithValue("@status", status);
            cmd.Parameters.AddWithValue("@keyCreditBureauKb", keyCreditBureauKb);
            await connect.OpenAsync(cancellationToken);
            await cmd.ExecuteNonQueryAsync(cancellationToken);
            await connect.CloseAsync();
        }

        public async Task UpdateRequestHistoryStatusAsync(string keyCreditBureauKb, string status, CancellationToken cancellationToken)
        {
            using var connect = new SqlConnection(_databaseSettings.CIBConnection);
            using var cmd = new SqlCommand(
                "UPDATE [dbo].[Request_History] SET [status] = @status WHERE [Key_ABS_Loan] = @keyCreditBureauKb",
                connect);
            cmd.Parameters.AddWithValue("@status", status);
            cmd.Parameters.AddWithValue("@keyCreditBureauKb", keyCreditBureauKb);
            await connect.OpenAsync(cancellationToken);
