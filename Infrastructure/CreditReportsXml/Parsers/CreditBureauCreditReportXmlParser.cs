...
public sealed class CreditBureauCreditReportXmlParser(ICreditReportXmlParserRepository creditReportXmlParserRepository, IOptions<StopFactorParserSettings> options, LogWriter logWriter) : ICreditReportXmlParser
{
    ...
    public async Task ParseAndPersistAsync(string KeyCreditBureauKb, string reportBase64, CancellationToken cancellationToken)
    {
        _logWriter.Log("TestParser.txt", $"Начало парсинга: Key={KeyCreditBureauKb}");
        // получаем app по loan_history_kb_key
        var app = await _creditReportXmlParserRepository.GetAppByKeyLoanHistoryKb(KeyCreditBureauKb, cancellationToken);
        ...
        try
        {
            // TODO: Implement the logic to parse the CreditBureau credit report XML and persist it using the repository.
        ...
        {
            _logWriter.Log("TestParser.txt", $"Завершение обработки Key={KeyCreditBureauKb}");
        }
    }
