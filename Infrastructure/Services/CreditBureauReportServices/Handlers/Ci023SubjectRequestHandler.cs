...
/// <summary>
/// Обработчик CI-023: Сведения о связанных субъектах
/// </summary>
public class Ci023SubjectRequestHandler : CiHandlerBase<CreditRegistrationSubjectRequest>
{
    public Ci023SubjectRequestHandler(
        ...
        ILogger<Ci023SubjectRequestHandler> logger)
    ...
    public override int CiCode => 23;

    public override async Task<CiProcessingResult> ProcessAsync(CancellationToken cancellationToken)
    {
        var requests = await CreditBureauReportRepository.GetCreditRegistrationSubjectRequestsAsync(cancellationToken);
        ...
        foreach (var item in requests)
        {
            ...
            if (item.Request is null)
            {
                ...
                await CreditBureauReportRepository.UpsertCiStatusAsync(
                    item.LoanKey, CiCode, 2, "CI-023 request is null", null, cancellationToken);
                continue;
            }
            ...
            try
            {
                ...
                var response = await RequestManagerService.SendPostRequest(
                    AsokiApplicationApiOptions.HostAddress + AsokiApplicationApiOptions.CreditRegistrationSubjectUrl,
                    baseRequest.ToJSON(),
                    item.LoanKey,
                    cancellationToken);
                ...
                if (string.IsNullOrWhiteSpace(response))
                {
                    ...
                    await CreditBureauReportRepository.UpsertCiStatusAsync(
                        item.LoanKey, CiCode, 2, "CI-023 returned empty response", null, cancellationToken);
                    continue;
                }
                ...
                {
                    result.Success++;
                    Logger.LogInformation("LoanKey:{LoanKey} CI-{CiCode} success. PLoanSubject={PLoanSubject}",
                        item.LoanKey, CiCode, item.Request.PLoanSubject);
                    LogWriter.Log("CreditRegistrationSubject.txt",
                        $"LoanKey:{item.LoanKey} CI-{CiCode:D3} success. PLoanSubject:{item.Request.PLoanSubject}");
                }
                else
                {
                    result.Error++;
