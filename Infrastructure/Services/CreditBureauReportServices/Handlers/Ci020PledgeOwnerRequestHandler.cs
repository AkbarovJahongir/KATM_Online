...
/// <summary>
/// Обработчик CI-020: Сведения о владельце залога
/// </summary>
public class Ci020PledgeOwnerRequestHandler : CiHandlerBase<CreditRegistrationPledgeOwner>
{
    public Ci020PledgeOwnerRequestHandler(
        ...
        ILogger<Ci020PledgeOwnerRequestHandler> logger)
    ...
    public override int CiCode => 20;

    public override async Task<CiProcessingResult> ProcessAsync(CancellationToken cancellationToken)
    {
        var requests = await CreditBureauReportRepository.CreditRegistrationPledgeOwnerAsync(cancellationToken);
        ...
        foreach (var item in requests)
        {
            ...
            if (item.Request is null)
            {
                ...
                await CreditBureauReportRepository.UpsertCiStatusAsync(
                    item.LoanKey, CiCode, 2, "CI-020 request is null", null, cancellationToken);
                continue;
            }
            ...
            try
            {
                ...
                var response = await RequestManagerService.SendPostRequest(
                    AsokiApplicationApiOptions.HostAddress + AsokiApplicationApiOptions.CreditPledgeOwnerUrl,
                    baseRequest.ToJSON(),
                    item.LoanKey,
                    cancellationToken);
                ...
                if (string.IsNullOrWhiteSpace(response))
                {
                    ...
                    await CreditBureauReportRepository.UpsertCiStatusAsync(
                        item.LoanKey, CiCode, 2, "CI-020 returned empty response", null, cancellationToken);
                    continue;
                }
