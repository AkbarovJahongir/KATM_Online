...
public class Ci016BankDetailRequestHandler : CiHandlerBase<CreditRegistrationBankDitailRequest>
{
    ...
    public override async Task<CiProcessingResult> ProcessAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var item in requests)
        {
            ...
            try
            {
                ...
                {
                    result.Error++;
                    result.AddDetail(item.LoanKey, false, "CI-016 returned empty response");
    ...
    {
        var requests = await CreditBureauReportRepository.GetCreditRegistrationBankDetailsRequestsByPeriodAsync(startDate, endDate, loanKey, cancellationToken);
